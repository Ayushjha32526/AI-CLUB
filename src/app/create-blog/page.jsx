...
const CreateBlog = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
          ...
          if (!res.ok) {
            throw new Error("Error occurred while saving the blog post");
          }
    ...
    onChange={(e) => {
        if (e.target.files.length > 0) {
            ...
            setPhoto(null); // Clear the photo if no file is selected
        }
    }}
/>

<button type="submit" className={classes.createBlog}>Create</button>
              </form>
            </div>
            <ToastContainer />
        </div>
