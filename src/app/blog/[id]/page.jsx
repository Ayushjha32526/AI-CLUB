...
const BlogDetails = ({ params }) => { // Changed from (ctx)
    const [blogDetails, setBlogDetails] = useState(null) // Changed from ""
    ...
    useEffect(() => {
        ...
        session && fetchBlog()
    }, [session, params.id]) // Added params.id to the dependencies

    useEffect(() => {
        async function fetchComments() {
            try {
                const res = await fetch(`/api/comment/${params.id}`, { cache: 'no-store' })
                if (!res.ok) {
    ...
    const handleDelete = async () => {
        try {
            const confirmModal = window.confirm("Are you sure you want to delete your blog?") // Changed from confirm
    ...
    const handleLike = async () => {
        try {
            const res = await fetch(`/api/blog/${params.id}/like`, {
                headers: {
                    'Authorization': `Bearer ${session?.user?.accessToken}`
                },
                method: 'PUT'
            ...
            if (res.ok) {
                ...
                setBlogLikes(prev => prev + (isLiked ? -1 : 1))
            }
        } catch (error) {
            console.error(error)
        }
    ...
    return (
        <div className={classes.container}>
            {blogDetails && ( // Render content only when blogDetails is available
                <div className={classes.wrapper}>
                    <Image src={blogDetails.imageUrl} width='750' height='650' />
                    <div className={classes.row}>
                        <h3 className={classes.title}>{blogDetails.title}</h3>
                        {session && blogDetails.authorId?._id.toString() === session?.user?._id.toString() ? (
                            <div className={classes.controls}>
                                <Link className={classes.editButton} href={`/blog/edit/${params.id}`}>
